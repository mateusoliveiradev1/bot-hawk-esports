...
describe('Security and Monitoring Integration', () => {
  ...
  beforeEach(() => {
    ...
    securityService = new SecurityService(databaseService, cacheService, logger, client, {
      rateLimit: {
        enabled: true,
        maxRequests: 10,
        windowMs: 60000,
  ...
  describe('Health Monitoring Integration', () => {
    it('should monitor security service health', async () => {
      ...
      expect(healthService.registerService).toHaveBeenCalledWith('security', expect.any(Function));
  ...
  describe('Metrics Collection Integration', () => {
    ...
    it('should track security incidents over time', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await securityService.logSecurityEvent('spam_detected', 'spammer-123', {
          messageCount: i + 1,
        });
      }
  ...
  describe('Performance Under Load', () => {
    ...
    it('should maintain accuracy under concurrent load', async () => {
      ...
      const promises = Array(20)
        .fill(null)
        .map(() => securityService.checkRateLimit(userId, 'command'));
