...
test.describe('Integração - Bot e Dashboard', () => {
  ...
  test('deve sincronizar dados entre bot e dashboard', async ({ page, request }) => {
    ...
    if (ticketId) {
      ...
      await expect(page.locator('text=' + botTicketData.title).first()).toBeVisible({ timeout: 15000 });
      ...
      if (await statusSelect.count() > 0) {
        ...
        // Salvar alteração
        const saveButton = page.locator('button:has-text("Salvar"), button:has-text("Save"), [data-testid="save-button"]').first();
  ...
  test('deve permitir gerenciar tickets do bot via dashboard', async ({ page, request }) => {
    ...
    if (ticketId) {
      ...
      if (await ticketLink.count() > 0) {
        ...
        const commentField = page.locator('textarea[name="comment"], textarea[placeholder*="comentário"], [data-testid="comment-input"]').first();
        
        if (await commentField.count() > 0) {
          await commentField.fill('Comentário adicionado via dashboard durante teste E2E');
          
          const submitButton = page.locator('button:has-text("Enviar"), button:has-text("Adicionar"), [data-testid="submit-comment"]').first();
        ...
        if (await prioritySelect.count() > 0) {
          ...
          // Salvar alteração
          const saveButton = page.locator('button:has-text("Salvar"), [data-testid="save-button"]').first();
        ...
        await expect(page.locator('text=Comentário adicionado via dashboard')).toBeVisible({ timeout: 10000 });
      }
    }
  });
    ...
    if (logsPageFound) {
      ...
      if (logCount > 0) {
        ...
        for (let i = 0; i < Math.min(logCount, 3); i++) {
          ...
          const hasTimestamp = await logEntry.locator('text=/\d{2}:\d{2}|\d{4}-\d{2}-\d{2}/').count() > 0;
      ...
      const logLevelFilter = page.locator('select[name*="level"], [data-testid="log-level"]').first();
  ...
  test('deve permitir controle do bot via dashboard', async ({ page, request }) => {
    ...
    if (controlPageFound) {
      ...
      const controlButtons = page.locator('button:has-text("Status"), button:has-text("Info"), [data-testid*="info"]');
