...
test.describe('Dashboard - Autenticação', () => {
  ...
  test('deve redirecionar usuário não autenticado de páginas protegidas', async ({ page }) => {
    ...
    for (const route of protectedRoutes) {
      ...
      const isRedirected =
        !currentUrl.includes(route) || currentUrl.includes('login') || currentUrl.includes('auth');
  ...
  test('deve ter segurança adequada', async ({ page }) => {
    ...
    const scriptContent = await page.evaluate(() => {
      ...
      return scripts.some(
        script => script.innerHTML.includes('eval(') || script.innerHTML.includes('document.write(')
      );
    });
