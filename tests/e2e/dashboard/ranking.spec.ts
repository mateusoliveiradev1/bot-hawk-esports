...
test.describe('Dashboard - Sistema de Ranking', () => {
    ...
    for (const selector of filterSelectors) {
      if (await page.locator(selector).count() > 0) {
        ...
        if (await filterElement.evaluate(el => el.tagName) === 'SELECT') {
          ...
          for (const option of options.slice(0, 3)) { // Testar até 3 opções
  ...
  test('deve paginar lista de ranking', async ({ page }) => {
    ...
    for (let i = 0; i < 20; i++) {
      additionalUsers.push(await helpers.createTestUser({
  ...
  test('deve ter responsividade adequada', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      if (viewport.width < 768) {
        ...
        const rankingItems = await page.locator('.ranking-item, .user-rank-item, .leaderboard-entry').count();
        if (rankingItems > 0) {
          const firstItem = page.locator('.ranking-item, .user-rank-item, .leaderboard-entry').first();
  ...
  test('deve atualizar dados em tempo real', async ({ page }) => {
    ...
    const rankingStillVisible = await page.locator('main, .ranking-container, .leaderboard').count() > 0;
