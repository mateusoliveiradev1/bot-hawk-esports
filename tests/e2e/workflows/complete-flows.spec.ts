...
test.describe('Fluxos Completos E2E', () => {
  ...
  test('fluxo completo: criação de ticket até resolução', async ({ page, request }) => {
    ...
    // 4. Criar novo ticket
    const createButton = page.locator('button:has-text("Criar"), button:has-text("Novo"), [data-testid="create-ticket"]').first();
    
    if (await createButton.count() > 0) {
      await createButton.click();
      
      // Preencher formulário
      await helpers.fillField('input[name="title"], [data-testid="ticket-title"]', 'Problema com Sistema de XP');
      await helpers.fillField('textarea[name="description"], [data-testid="ticket-description"]', 'O sistema de XP não está funcionando corretamente. Usuários não estão recebendo XP por mensagens.');
      ...
      // Submeter formulário
      const submitButton = page.locator('button[type="submit"], button:has-text("Criar"), [data-testid="submit-ticket"]').first();
      ...
      const commentField = page.locator('textarea[name="comment"], [data-testid="comment-input"]').first();
      
      if (await commentField.count() > 0) {
        await commentField.fill('Investigando o problema. Verificando logs do sistema de XP.');
        
        const addCommentButton = page.locator('button:has-text("Adicionar"), button:has-text("Enviar"), [data-testid="add-comment"]').first();
      ...
      if (await statusSelect.count() > 0) {
        ...
        const saveButton = page.locator('button:has-text("Salvar"), [data-testid="save-status"]').first();
      ...
      if (await commentField.count() > 0) {
        await commentField.fill('Problema identificado: configuração incorreta no módulo de XP. Aplicando correção.');
      ...
      if (await commentField.count() > 0) {
        await commentField.fill('Ticket resolvido. Sistema de XP corrigido e funcionando normalmente.');
  ...
  test('fluxo completo: novo usuário no sistema', async ({ page, request }) => {
    ...
    if (await rankingUser.count() === 0) {
      ...
      const searchField = page.locator('input[name="search"], [data-testid="ranking-search"]').first();
      if (await searchField.count() > 0) {
        await searchField.fill(newUser.username);
        await page.waitForTimeout(1000);
      }
  ...
  test('fluxo completo: moderação de usuário', async ({ page, request }) => {
    ...
    if (await userToModerate.count() > 0) {
      ...
      const warnButton = page.locator('button:has-text("Advertir"), button:has-text("Warn"), [data-testid="warn-user"]').first();
      
      if (await warnButton.count() > 0) {
        await warnButton.click();
        
        // Preencher motivo da advertência
        const reasonField = page.locator('textarea[name="reason"], [data-testid="warn-reason"]').first();
        if (await reasonField.count() > 0) {
          await reasonField.fill('Comportamento inadequado no chat');
          
          const confirmButton = page.locator('button:has-text("Confirmar"), button:has-text("Advertir")').first();
      ...
      const timeoutButton = page.locator('button:has-text("Timeout"), button:has-text("Silenciar"), [data-testid="timeout-user"]').first();
      
      if (await timeoutButton.count() > 0) {
        await timeoutButton.click();
        
        // Selecionar duração
        const durationSelect = page.locator('select[name="duration"], [data-testid="timeout-duration"]').first();
        if (await durationSelect.count() > 0) {
          await durationSelect.selectOption('1h');
          
          const confirmTimeoutButton = page.locator('button:has-text("Confirmar"), button:has-text("Aplicar")').first();
  ...
  test('fluxo completo: configuração do sistema', async ({ page, request }) => {
    ...
    for (const url of configUrls) {
      try {
        ...
        const hasConfigElements = await page.locator('input[name*="prefix"], input[name*="channel"], select[name*="language"]').count() > 0;
    ...
    if (configFound) {
      ...
      const logChannelField = page.locator('input[name="logChannel"], [data-testid="log-channel"]').first();
      ...
      const xpEnabledCheckbox = page.locator('input[name="xpEnabled"], [data-testid="xp-enabled"]').first();
      ...
      const advancedTab = page.locator('button:has-text("Avançado"), [data-testid="advanced-tab"]').first();
      ...
      if (await advancedTab.count() > 0) {
        ...
        const advancedFields = page.locator('input[name*="rate"], input[name*="limit"], input[name*="timeout"]');
  ...
  test('fluxo completo: backup e restauração', async ({ page, request }) => {
    ...
    for (const url of backupUrls) {
      try {
        ...
        const hasBackupElements = await page.locator('button:has-text("Backup"), button:has-text("Export"), [data-testid*="backup"]').count() > 0;
    ...
    if (backupPageFound) {
      // 3. Criar backup
      const createBackupButton = page.locator('button:has-text("Criar Backup"), button:has-text("Create Backup"), [data-testid="create-backup"]').first();
      ...
      if (itemCount > 0) {
        ...
        const hasTimestamp = await firstBackup.locator('text=/\d{2}\/\d{2}\/\d{4}|\d{4}-\d{2}-\d{2}/').count() > 0;
        ...
        const downloadButton = firstBackup.locator('button:has-text("Download"), [data-testid="download-backup"]').first();
        ...
        const restoreButton = firstBackup.locator('button:has-text("Restaurar"), button:has-text("Restore"), [data-testid="restore-backup"]').first();
  ...
  test('fluxo completo: monitoramento e alertas', async ({ page, request }) => {
    ...
    for (const url of monitoringUrls) {
      try {
        ...
        const hasMonitoringElements = await page.locator('text=/CPU|Memory|Latency|Uptime/, [data-testid*="metric"]').count() > 0;
    ...
    if (monitoringPageFound) {
      ...
      if (await alertsSection.count() > 0) {
        ...
        if (alertCount > 0) {
          ...
          const hasSeverity = await firstAlert.locator('text=/critical|warning|info|error/i').count() > 0;
          
          // Deve ter timestamp
          const hasTimestamp = await firstAlert.locator('text=/\d{2}:\d{2}|\d{2}\/\d{2}/').count() > 0;
      ...
      const createAlertButton = page.locator('button:has-text("Criar Alerta"), button:has-text("New Alert"), [data-testid="create-alert"]').first();
      ...
      if (await createAlertButton.count() > 0) {
        ...
        const conditionField = page.locator('select[name="condition"], [data-testid="alert-condition"]').first();
        if (await conditionField.count() > 0) {
          await conditionField.selectOption('cpu_usage');
        }
        
        const thresholdField = page.locator('input[name="threshold"], [data-testid="alert-threshold"]').first();
        if (await thresholdField.count() > 0) {
          await thresholdField.fill('80');
        }
        
        // Salvar alerta
        const saveAlertButton = page.locator('button:has-text("Salvar"), button:has-text("Create")').first();
