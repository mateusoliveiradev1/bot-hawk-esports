...
test.describe('Testes de Performance e Stress', () => {
  ...
  test('performance: navegação entre páginas', async ({ page }) => {
    ...
    const pages = ['/tickets', '/ranking', '/users', '/'];
  ...
  test('stress: múltiplas requisições simultâneas', async ({ page, request }) => {
    ...
    for (let i = 0; i < concurrentRequests; i++) {
      requests.push(request.get(`${API_URL}/api/health`).catch(() => null));
    }
    ...
    console.log(
      `Stress Test: ${successfulRequests}/${concurrentRequests} requests successful in ${totalTime}ms`
    );
  });
  ...
  test('stress: criação rápida de múltiplos tickets', async ({ page, request }) => {
    ...
    for (let i = 0; i < ticketCount; i++) {
      ...
      const createButton = page
        .locator('button:has-text("Criar"), button:has-text("Novo")')
        .first();
  ...
  test('performance: ranking com muitos usuários', async ({ page, request }) => {
    ...
    const nextPageButton = page
      .locator('button:has-text("Próximo"), button:has-text("Next"), [data-testid="next-page"]')
      .first();
  ...
  test('stress: navegação rápida entre páginas', async ({ page }) => {
    const pages = ['/', '/tickets', '/ranking', '/users', '/tickets', '/', '/ranking'];
  ...
  test('performance: API response times', async ({ request }) => {
    const endpoints = ['/api/health', '/api/tickets', '/api/users', '/api/stats'];
