...
import { Search, Filter, Command, Clock, AlertCircle, CheckCircle, BarChart3 } from 'lucide-react';
...
export default function Commands() {
  ...
  const avgSuccessRate =
    commands?.reduce((sum, cmd) => sum + cmd.successRate, 0) / (commands?.length || 1) || 0;
  const avgResponseTime =
    commands?.reduce((sum, cmd) => sum + cmd.avgResponseTime, 0) / (commands?.length || 1) || 0;
    ...
    <div className='space-y-6'>
                ...
                <tr key={command.id} className='hover:bg-secondary-50'>
                  <td className='px-6 py-4 whitespace-nowrap'>
                    <div>
                      ...
                      <div className='text-sm text-secondary-500 mt-1'>{command.description}</div>
                    </div>
                  </td>
                  <td className='px-6 py-4 whitespace-nowrap'>
                    <span
                  ...
                  <td className='px-6 py-4 whitespace-nowrap'>
                    <div className='flex items-center'>
                      <span
                        className={`text-sm font-medium ${getSuccessRateColor(command.successRate)}`}
                      >
                        {command.successRate}%
                      </span>
                    </div>
                  ...
                  <td className='px-6 py-4 whitespace-nowrap'>
                    <span
                      className={`text-sm font-medium ${getResponseTimeColor(command.avgResponseTime)}`}
                    >
      ...
      {filteredCommands.length === 0 && (
        <div className='card text-center py-12'>
          ...
          <h3 className='mt-4 text-lg font-medium text-secondary-900'>Nenhum comando encontrado</h3>
