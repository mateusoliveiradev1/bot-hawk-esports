...
import { Search, Filter, MoreVertical, Shield, User, Calendar, Activity } from 'lucide-react';
...
export default function Users() {
  ...
  const filteredUsers = users.filter(userGuild => {
    ...
    const isOnline =
      user.lastSeen && new Date(user.lastSeen) > new Date(Date.now() - 5 * 60 * 1000);
      ...
      <div className='card'>
        {error && (
          <div className='px-6 py-4 border-b border-secondary-200'>
            <div className='text-sm text-red-600'>{error} - Exibindo dados de exemplo</div>
          </div>
        )}
                  ...
                  return (
                    <tr key={user.id} className='hover:bg-secondary-50'>
                      ...
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <div className='flex items-center'>
                          <div
                            className={`h-2 w-2 rounded-full mr-2 ${statusColors[status as keyof typeof statusColors] || 'bg-secondary-400'}`}
                          />
