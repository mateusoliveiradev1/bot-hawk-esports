...
async function testBackupSystem() {
  ...
  try {
    ...
    const backupService = new BackupService(prisma, logger, healthService, metricsService, config.backup);
    ...
    try {
      ...
      console.log(`  - Último backup: ${stats.lastBackup ? new Date(stats.lastBackup).toLocaleString() : 'Nenhum'}`);
