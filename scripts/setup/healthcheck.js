    ...
    try {
      ...
      if (res.statusCode === 200 && response.success) {
        ...
        if (healthData.system) {
          console.log(
            `Memory: ${healthData.system.memory.used}MB/${healthData.system.memory.total}MB (${healthData.system.memory.percentage}%)`
          );
          console.log(
            `Discord: ${healthData.system.discord.connected ? 'Connected' : 'Disconnected'} - ${healthData.system.discord.guilds} guilds, ${healthData.system.discord.users} users`
          );
        }
