...
import {
  ...
  AdaptiveChallenge,
  PlayerStats,
} from '../../services/adaptive-challenge.service';
...
class AdaptiveChallengeCommand extends BaseCommand {
  ...
  private async handleMyChallenges(
    ...
    if (challenges.length === 0) {
      ...
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(generateButton);
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(refreshButton, profileButton);
  ...
  private async handlePreferences(
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(comingSoonButton, backButton);
