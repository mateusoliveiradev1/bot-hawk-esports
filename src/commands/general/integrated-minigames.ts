...
import {
  IntegratedMiniGamesService,
  MiniGameDefinition,
  MiniGameSession,
} from '../../services/integrated-minigames.service';
...
class IntegratedMinigamesCommand extends BaseCommand {
  ...
  private async handleCreateSession(
    ...
    const difficulty =
      (interaction.options.getString('dificuldade') as 'easy' | 'medium' | 'hard' | 'extreme') ||
      'medium';
    ...
    if (maxParticipants) {
      customSettings.maxParticipants = maxParticipants;
    }
    if (duration) {
  ...
  private async handleRanking(
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value:
          'O sistema de ranking está sendo implementado e estará disponível em breve!\n\nRecursos planejados:\n• Rankings por jogo\n• Rankings globais\n• Histórico de desempenho\n• Temporadas competitivas',
        inline: false,
      })
      .setColor(0xffa500)
      .setTimestamp();
