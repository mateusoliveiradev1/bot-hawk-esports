...
class QuizCommand extends BaseCommand {
    ...
    try {
      ...
      if (!session) {
        const embed = new EmbedBuilder()
          ...
          .addFields({
            name: '💡 Dica',
            value: 'Verifique se não há outro quiz ativo neste canal',
            inline: false,
          })
          .setTimestamp();
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .addFields({
          ...
          value: 'Use `/help quiz` para mais informações',
          inline: false,
        })
        .setTimestamp();
  ...
  private getDifficultyInfo(difficulty: string): {
    name: string;
    emoji: string;
    multiplier: number;
  } {
        ...
        .addFields(
          {
            name: '🏆 Participantes',
            value:
              participants.map((p: any, i: number) => `${i + 1}. ${p.username}`).join('\n') ||
              'Nenhum',
            inline: true,
          },
          {
