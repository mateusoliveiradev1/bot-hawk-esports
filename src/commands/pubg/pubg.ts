...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
    ...
    try {
      switch (subcommand) {
        case 'link':
          await handleLinkCommand(interaction, client);
          break;
        case 'stats':
          await handleStatsCommand(interaction, client);
          break;
        case 'compare':
          await handleCompareCommand(interaction, client);
          break;
        default:
      ...
      logger.error(`Error executing PUBG command ${subcommand}:`, error);
...
/**
 * Handle /pubg link command
 */
async function handleLinkCommand(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
  ...
  try {
    ...
    // Verificar se o jogador existe na API do PUBG
    logger.info(`Searching for PUBG player: ${username} on ${platform}`);
    const player = await pubgService.getPlayerByName(username, platform);
    
    if (!player) {
      await interaction.editReply({
        content: `❌ Jogador **${username}** não encontrado na plataforma **${platform}**. Verifique se o nome está correto.`,
      });
      return;
    }

    // Verificar se a conta já está vinculada a outro usuário
    const existingUser = await client.database.client.user.findFirst({
        where: {
          pubgUsername: username,
    ...
    // Salvar ou atualizar a vinculação
    ...
    const stats = await pubgService.getPlayerStats(player.id, platform);
    if (stats) {
      if (badgeService) {
        await badgeService.onPUBGStatsUpdated(userId, stats);
      }
    }

    const embed = new EmbedBuilder()
      .setTitle('🔗 Conta PUBG Vinculada!')
      .setDescription(`Sua conta **${player.name}** (${platform.toUpperCase()}) foi vinculada com sucesso!`)
      .addFields(
    ...
    logger.error(`Error linking PUBG account for user ${userId}:`, error);
...
/**
 * Handle /pubg stats command
 */
async function handleStatsCommand(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
...
/**
 * Handle /pubg compare command
 */
async function handleCompareCommand(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
