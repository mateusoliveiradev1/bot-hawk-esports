...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('status')
        .setDescription('Ver status das melhorias de presença')
    )
    .addSubcommand(subcommand =>
      subcommand
        ...
        .addUserOption(option =>
          option
            .setName('usuario')
            .setDescription('Usuário para validar')
            .setRequired(true)
        )
    )
    .addSubcommand(subcommand =>
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('stats')
        .setDescription('Ver estatísticas das melhorias')
    )
    .addSubcommand(subcommand =>
      subcommand
  ...
  async execute(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
    try {
      ...
      const enhancementsService = (client as any).presenceEnhancementsService as PresenceEnhancementsService;
...
async function handleConfigPunishment(
  ...
  if (interval !== null) config.checkInterval = interval;
  if (gracePeriod !== null) config.gracePeriod = gracePeriod;
  ...
  const configFields = [];
  if (enabled !== null) configFields.push(`**Habilitado:** ${enabled ? 'Sim' : 'Não'}`);
  if (interval !== null) configFields.push(`**Intervalo:** ${interval} minutos`);
  if (gracePeriod !== null) configFields.push(`**Período de Graça:** ${gracePeriod} minutos`);
...
async function handleUserHistory(
  ...
  const historyText = sortedEnhancements.map(enhancement => {
    const typeEmoji = {
      'pubg_validation': '🎮',
      'auto_punishment': '⚖️',
