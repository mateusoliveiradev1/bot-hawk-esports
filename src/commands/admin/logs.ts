  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        ...
        .addBooleanOption(option =>
          option
            .setName('ignorar_bots')
            .setDescription('Ignorar ações de bots')
            .setRequired(false)
        )
    )
    .addSubcommand(subcommand =>
      subcommand
        .setName('status')
        .setDescription('Ver status atual do sistema de logs')
    )
    .addSubcommand(subcommand =>
      subcommand
        .setName('teste')
        .setDescription('Enviar mensagem de teste para os canais de log')
    )
    .addSubcommand(subcommand =>
      subcommand
        .setName('changelog')
...
async function handleConfigCommand(
  ...
  if (moderacaoChannel) updates['channels.moderation'] = moderacaoChannel.id;
  if (mensagensChannel) updates['channels.messages'] = mensagensChannel.id;
  if (membrosChannel) updates['channels.members'] = membrosChannel.id;
  if (vozChannel) updates['channels.voice'] = vozChannel.id;
  if (servidorChannel) updates['channels.server'] = servidorChannel.id;
  if (changelogChannel) updates['channels.changelog'] = changelogChannel.id;
  ...
  if (moderacaoChannel) fields.push({ name: '🛡️ Moderação', value: `<#${moderacaoChannel.id}>`, inline: true });
  if (mensagensChannel) fields.push({ name: '💬 Mensagens', value: `<#${mensagensChannel.id}>`, inline: true });
  if (membrosChannel) fields.push({ name: '👥 Membros', value: `<#${membrosChannel.id}>`, inline: true });
  if (vozChannel) fields.push({ name: '🔊 Voz', value: `<#${vozChannel.id}>`, inline: true });
  if (servidorChannel) fields.push({ name: '🏠 Servidor', value: `<#${servidorChannel.id}>`, inline: true });
  if (changelogChannel) fields.push({ name: '📋 Changelog', value: `<#${changelogChannel.id}>`, inline: true });
...
async function handleEventosCommand(
  ...
  if (acoesModeracao !== null) newEvents.moderationActions = acoesModeracao;
  if (eventosVoz !== null) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x00ff00)
    .addFields(
      { name: '🗑️ Mensagens Deletadas', value: newEvents.messageDelete ? '✅ Ativo' : '❌ Inativo', inline: true },
      { name: '✏️ Mensagens Editadas', value: newEvents.messageEdit ? '✅ Ativo' : '❌ Inativo', inline: true },
      { name: '👋 Entrada de Membros', value: newEvents.memberJoin ? '✅ Ativo' : '❌ Inativo', inline: true },
      { name: '👋 Saída de Membros', value: newEvents.memberLeave ? '✅ Ativo' : '❌ Inativo', inline: true },
      { name: '👤 Atualização de Membros', value: newEvents.memberUpdate ? '✅ Ativo' : '❌ Inativo', inline: true },
      { name: '🛡️ Ações de Moderação', value: newEvents.moderationActions ? '✅ Ativo' : '❌ Inativo', inline: true },
      { name: '🔊 Eventos de Voz', value: newEvents.voiceJoin ? '✅ Ativo' : '❌ Inativo', inline: true }
    )
    .setTimestamp();
...
async function handleFiltrosCommand(
  ...
  if (ignorarBots !== null) newFilters.ignoreBots = ignorarBots;
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x00ff00)
    .addFields(
      { name: '🤖 Ignorar Bots', value: newFilters.ignoreBots ? '✅ Ativo' : '❌ Inativo', inline: true }
    )
    .setTimestamp();
...
async function handleStatusCommand(
  ...
  if (config.channels.moderation) channels.push(`🛡️ Moderação: <#${config.channels.moderation}>`);
  if (config.channels.messages) channels.push(`💬 Mensagens: <#${config.channels.messages}>`);
  if (config.channels.members) channels.push(`👥 Membros: <#${config.channels.members}>`);
  if (config.channels.voice) channels.push(`🔊 Voz: <#${config.channels.voice}>`);
  if (config.channels.server) channels.push(`🏠 Servidor: <#${config.channels.server}>`);
  if (config.channels.changelog) channels.push(`📋 Changelog: <#${config.channels.changelog}>`);
  
  if (channels.length > 0) {
    embed.addFields({ name: '📍 Canais Configurados', value: channels.join('\n'), inline: false });
  } else {
    embed.addFields({ name: '📍 Canais Configurados', value: 'Nenhum canal configurado', inline: false });
  }
  
  // Eventos ativos
  const activeEvents: string[] = [];
  if (config.events.messageDelete) activeEvents.push('🗑️ Mensagens Deletadas');
  if (config.events.messageEdit) activeEvents.push('✏️ Mensagens Editadas');
  if (config.events.memberJoin) activeEvents.push('👋 Entrada de Membros');
  if (config.events.memberLeave) activeEvents.push('👋 Saída de Membros');
  if (config.events.memberUpdate) activeEvents.push('👤 Atualização de Membros');
  if (config.events.moderationActions) activeEvents.push('🛡️ Ações de Moderação');
  if (config.events.voiceJoin) activeEvents.push('🔊 Eventos de Voz');
...
async function handleTesteCommand(
  ...
  const testEmbed = new EmbedBuilder()
    ...
    .setDescription('Esta é uma mensagem de teste para verificar se o sistema de logs está funcionando corretamente.')
    .setColor(0x00ff00)
    .addFields(
  ...
  const resultEmbed = new EmbedBuilder()
    ...
    .setDescription(testResults.length > 0 ? testResults.join('\n') : 'Nenhum canal configurado para teste.')
...
async function handleChangelogCommand(
  ...
  const tipo = interaction.options.getString('tipo', true) as 'feature' | 'bugfix' | 'improvement' | 'breaking';
