...
class LogsCommand extends BaseCommand {
  ...
  private async handleConfigCommand(
    ...
    if (moderacao) {
      newChannels.moderation = moderacao.id;
    }
    if (mensagens) {
      newChannels.messages = mensagens.id;
    }
    if (membros) {
      newChannels.members = membros.id;
    }
    if (voz) {
      newChannels.voice = voz.id;
    }
    if (servidor) {
      newChannels.server = servidor.id;
    }
    if (changelog) {
      newChannels.changelog = changelog.id;
    }
  ...
  private async handleEventosCommand(
    ...
    if (acoesMoederacao !== null) {
      newEvents.moderationActions = acoesMoederacao;
    }
    if (eventosVoz !== null) {
