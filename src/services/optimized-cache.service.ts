...
import { getRedisProductionConfig, getRedisOptions, redisHealthCheckConfig, redisMonitoringConfig } from '../config/redis-production.config';
...
export class OptimizedCacheService {
  ...
  private initializeClient(): void {
    ...
    try {
      if (this.config.performance.clustering.enabled && this.config.performance.clustering.nodes.length > 0) {
        // Redis Cluster
  ...
  private getTTLForKey(key: string): number {
    if (key.startsWith('user:')) {return this.config.ttl.user;}
    if (key.startsWith('guild:')) {return this.config.ttl.guild;}
    if (key.startsWith('pubg:player:')) {return this.config.ttl.pubgPlayer;}
    if (key.startsWith('pubg:stats:')) {return this.config.ttl.pubgStats;}
    if (key.startsWith('ranking:')) {return this.config.ttl.ranking;}
    if (key.startsWith('leaderboard:')) {return this.config.ttl.leaderboard;}
    if (key.startsWith('cooldown:')) {return this.config.ttl.cooldown;}
    if (key.startsWith('session:')) {return this.config.ttl.session;}
    if (key.startsWith('music:')) {return this.config.ttl.musicQueue;}
    if (key.startsWith('quiz:')) {return this.config.ttl.quiz;}
    if (key.startsWith('clip:')) {return this.config.ttl.clip;}
    if (key.startsWith('badge:')) {return this.config.ttl.badge;}
      ...
      try {
        ...
        if (stats.memory.usage > this.config.monitoring.memoryWarningThreshold) {
          this.logger.warn('Memory cache usage high', {
            category: LogCategory.CACHE,
            metadata: { usage: stats.memory.usage, threshold: this.config.monitoring.memoryWarningThreshold },
          });
        }
      } catch (error) {
