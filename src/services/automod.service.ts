...
export class AutoModerationService {
  ...
  private messageHistory: Map<string, Array<{ content: string; timestamp: number; channelId: string }>> = new Map();
  ...
  private isExempt(message: Message, config: AutoModConfig): boolean {
    ...
    if (Array.isArray(config.exemptions.users) && config.exemptions.users.includes(message.author.id)) {
      return true;
    }

    // Verifica canais isentos
    if (Array.isArray(config.exemptions.channels) && config.exemptions.channels.includes(message.channel.id)) {
      return true;
    }
    ...
    if (Array.isArray(config.exemptions.roles) && member.roles?.cache) {
      const hasExemptRole = member.roles.cache.some(role => 
        role?.id && config.exemptions.roles.includes(role.id)
      );
  ...
  private async checkSpam(message: Message, config: AutoModConfig): Promise<ModerationResult> {
    ...
    try {
      ...
      const recentMessages = userMessages.filter(
        msg => msg?.createdTimestamp && (now - msg.createdTimestamp < config.spamDetection.timeWindow * 1000)
      );
  ...
  private checkProfanity(message: Message, config: AutoModConfig): ModerationResult {
    ...
    try {
      ...
      if (!content) return { violated: false };

      const customWords = Array.isArray(config.profanityFilter.customWords) ? config.profanityFilter.customWords : [];
      const allProfanity = [
      ...
      for (const word of allProfanity) {
        if (!word || word.length < 2) continue; // Evita palavras muito curtas
          ...
          try {
            if (pattern.test(content)) {
              ...
              if (urls && Array.isArray(urls)) {
                const whitelist = Array.isArray(config.linkFilter.whitelist) ? config.linkFilter.whitelist : [];
  ...
  private async handleViolation(message: Message, result: ModerationResult, config: AutoModConfig): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Handled violation for user ${userId}: ${result.violationType} - ${result.punishment}`);
    ...
    try {
      ...
      switch (punishment) {
        ...
        case PunishmentType.MUTE:
          ...
          this.logger.info(`Muted ${member.user.tag} for ${config.punishments.mute?.duration || 10} minutes: ${reason}`);
          break;
    ...
    try {
      ...
      if (message.content && message.content.trim()) {
        const content = message.content.length > 1000 
          ...
          : message.content;
        embed.addFields({ name: 'Conteúdo da Mensagem', value: `\`\`\`${content}\`\`\``, inline: false });
      }
      
      await logChannel.send({ embeds: [embed] });
      this.logger.debug(`Logged violation for user ${message.author.id} in guild ${message.guild.id}`);
  ...
  private cleanupOldData(): void {
    try {
      ...
      for (const [userId, messages] of this.userMessageHistory.entries()) {
        ...
        const recentMessages = messages.filter(msg => {
          return msg && typeof msg.createdTimestamp === 'number' && (now - msg.createdTimestamp) < maxAge;
        });
      ...
      if (cleanedUsers > 0 || cleanedMessages > 0 || cleanedViolations > 0) {
        this.logger.info(`Cleaned up automod data: ${cleanedUsers} users, ${cleanedMessages} messages, ${cleanedViolations} violations`);
      } else {
    ...
    try {
      ...
      if (config.spamDetection && typeof config.spamDetection === 'object') {
        validatedConfig.spamDetection = {
          enabled: typeof config.spamDetection.enabled === 'boolean' ? config.spamDetection.enabled : true,
          maxMessages: typeof config.spamDetection.maxMessages === 'number' && config.spamDetection.maxMessages > 0 
            ? Math.min(config.spamDetection.maxMessages, 20) : 5,
          timeWindow: typeof config.spamDetection.timeWindow === 'number' && config.spamDetection.timeWindow > 0 
            ? Math.min(config.spamDetection.timeWindow, 300) : 10,
          maxDuplicates: typeof config.spamDetection.maxDuplicates === 'number' && config.spamDetection.maxDuplicates > 0 
            ? Math.min(config.spamDetection.maxDuplicates, 10) : 3,
          duplicateTimeWindow: typeof config.spamDetection.duplicateTimeWindow === 'number' && config.spamDetection.duplicateTimeWindow > 0 
            ? Math.min(config.spamDetection.duplicateTimeWindow, 600) : 60
        };
      }
      ...
      if (config.profanityFilter && typeof config.profanityFilter === 'object') {
        validatedConfig.profanityFilter = {
          enabled: typeof config.profanityFilter.enabled === 'boolean' ? config.profanityFilter.enabled : true,
          strictMode: typeof config.profanityFilter.strictMode === 'boolean' ? config.profanityFilter.strictMode : false,
          customWords: Array.isArray(config.profanityFilter.customWords) 
            ? config.profanityFilter.customWords.filter(word => typeof word === 'string' && word.trim().length > 0)
            : []
        };
      }
      ...
      if (config.linkFilter && typeof config.linkFilter === 'object') {
        validatedConfig.linkFilter = {
          enabled: typeof config.linkFilter.enabled === 'boolean' ? config.linkFilter.enabled : true,
          allowWhitelisted: typeof config.linkFilter.allowWhitelisted === 'boolean' ? config.linkFilter.allowWhitelisted : true,
          blockInvites: typeof config.linkFilter.blockInvites === 'boolean' ? config.linkFilter.blockInvites : true,
          blockSuspicious: typeof config.linkFilter.blockSuspicious === 'boolean' ? config.linkFilter.blockSuspicious : true,
          whitelist: Array.isArray(config.linkFilter.whitelist) 
            ? config.linkFilter.whitelist.filter(domain => typeof domain === 'string' && domain.trim().length > 0)
            : [],
          blacklist: Array.isArray(config.linkFilter.blacklist) 
            ? config.linkFilter.blacklist.filter(domain => typeof domain === 'string' && domain.trim().length > 0)
            : []
        };
      }
      ...
      if (config.capsFilter && typeof config.capsFilter === 'object') {
        validatedConfig.capsFilter = {
          enabled: typeof config.capsFilter.enabled === 'boolean' ? config.capsFilter.enabled : true,
          maxPercentage: typeof config.capsFilter.maxPercentage === 'number' && config.capsFilter.maxPercentage > 0 && config.capsFilter.maxPercentage <= 100
            ? config.capsFilter.maxPercentage : 70,
          minLength: typeof config.capsFilter.minLength === 'number' && config.capsFilter.minLength > 0
            ? Math.min(config.capsFilter.minLength, 100) : 5
        };
      }
      ...
      if (config.escalation && typeof config.escalation === 'object') {
        validatedConfig.escalation = {
          enabled: typeof config.escalation.enabled === 'boolean' ? config.escalation.enabled : true,
          warnThreshold: typeof config.escalation.warnThreshold === 'number' && config.escalation.warnThreshold > 0
            ? config.escalation.warnThreshold : 1,
          muteThreshold: typeof config.escalation.muteThreshold === 'number' && config.escalation.muteThreshold > 0
            ? config.escalation.muteThreshold : 3,
          kickThreshold: typeof config.escalation.kickThreshold === 'number' && config.escalation.kickThreshold > 0
            ? config.escalation.kickThreshold : 5,
          banThreshold: typeof config.escalation.banThreshold === 'number' && config.escalation.banThreshold > 0
            ? config.escalation.banThreshold : 10,
          resetTime: typeof config.escalation.resetTime === 'number' && config.escalation.resetTime > 0
            ? config.escalation.resetTime : 24
        };
      }
      ...
      if (config.logging && typeof config.logging === 'object') {
        validatedConfig.logging = {
          ...
          channelId: typeof config.logging.channelId === 'string' && config.logging.channelId.trim().length > 0
            ? config.logging.channelId.trim() : '',
          logWarnings: typeof config.logging.logWarnings === 'boolean' ? config.logging.logWarnings : true,
          ...
          logDeletedMessages: typeof config.logging.logDeletedMessages === 'boolean' ? config.logging.logDeletedMessages : true
        };
      }
      ...
      if (config.exemptions && typeof config.exemptions === 'object') {
        validatedConfig.exemptions = {
          ...
          channels: Array.isArray(config.exemptions.channels) 
            ? config.exemptions.channels.filter(id => typeof id === 'string' && id.trim().length > 0)
            : []
        };
      }
  ...
  public getStats(): { 
    ...
    try {
      ...
      const activeGuilds = Array.from(this.guildConfigs.keys())
        .filter(guildId => typeof guildId === 'string' && guildId.length > 0);
