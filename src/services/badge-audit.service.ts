...
export class BadgeAuditService {
   ...
   */
  private async findDuplicatedUserBadges(): Promise<Array<{ userId: string; badgeId: string; count: number }>> {
    try {
      const duplicates = await this.database.client.$queryRaw<Array<{ userId: string; badgeId: string; count: bigint }>>(
        Prisma.sql`
  ...
  private async findInvalidBadgeReferences(): Promise<Array<{ userId: string; badgeId: string }>> {
    try {
      ...
      const validBadgeIds = new Set(
        (await this.database.client.badge.findMany({ select: { id: true } }))
          .map(b => b.id)
      );
    ...
    try {
      ...
      for (const serviceBadge of serviceBadges) {
        ...
        if (dbBadge.category !== serviceBadge.category) {
          issues.push(`Category mismatch: DB='${dbBadge.category}' vs Service='${serviceBadge.category}'`);
        }
        
        if (dbBadge.rarity !== serviceBadge.rarity) {
          issues.push(`Rarity mismatch: DB='${dbBadge.rarity}' vs Service='${serviceBadge.rarity}'`);
        }
        
        if (dbBadge.isActive !== serviceBadge.isActive) {
          issues.push(`Active status mismatch: DB=${dbBadge.isActive} vs Service=${serviceBadge.isActive}`);
        }
        
        if (dbBadge.isSecret !== serviceBadge.isSecret) {
          issues.push(`Secret status mismatch: DB=${dbBadge.isSecret} vs Service=${serviceBadge.isSecret}`);
        }
    ...
    try {
      ...
      for (const duplicate of auditResults.duplicatedUserBadges) {
        ...
        if (userBadges.length > 1) {
          ...
          this.logger.info(`Removed ${toDelete.length} duplicate badges for user ${duplicate.userId}, badge ${duplicate.badgeId}`);
        }
      }
      ...
      for (const invalid of auditResults.invalidBadgeReferences) {
        ...
        fixedCount++;
        this.logger.info(`Removed invalid badge reference: user ${invalid.userId}, badge ${invalid.badgeId}`);
      }
   ...
   */
  public async cleanupOrphanedBadges(orphanedBadgeIds: string[], confirm: boolean = false): Promise<number> {
    if (!confirm) {
      this.logger.warn('Orphaned badge cleanup requires confirmation. Set confirm=true to proceed.');
      return 0;
    }
      ...
      for (const badge of badges) {
        ...
        try {
          ...
          for (const req of requirements) {
            ...
            if (req.operator === 'between' && (!Array.isArray(req.value) || req.value.length !== 2)) {
