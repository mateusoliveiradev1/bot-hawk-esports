...
export class PresenceEnhancementsService {
   ...
   */
  public async validatePUBGIntegration(userId: string, guildId: string): Promise<PUBGPresenceValidation> {
    try {
      let validation = this.pubgValidations.get(userId);
      
      // Check if validation is recent
      if (validation && (Date.now() - validation.lastValidated.getTime()) < this.validationCacheTime) {
        return validation;
      }
  ...
  public async enhancedCheckIn(
    ...
    try {
      ...
      const checkInResult = await this.presenceService.checkIn(
        guildId,
        userId,
        location,
        note
  ...
  public async enhancedCheckOut(
    ...
    try {
      ...
      const checkOutResult = await this.presenceService.checkOut(
        guildId,
        userId,
        note
      );
  ...
  private calculatePUBGBonus(stats: NonNullable<PUBGPresenceValidation['stats']>): number {
    ...
    // KDA bonus
    if (stats.kda >= 2.0) bonus += 15;
    else if (stats.kda >= 1.5) bonus += 10;
    else if (stats.kda >= 1.0) bonus += 5;
    
    // Win rate bonus (assuming 10% is good)
    const winRate = stats.matches > 0 ? (stats.wins / stats.matches) * 100 : 0;
    if (winRate >= 20) bonus += 20;
    else if (winRate >= 15) bonus += 15;
    else if (winRate >= 10) bonus += 10;
    else if (winRate >= 5) bonus += 5;
   ...
   */
  private calculateSessionBonus(stats: NonNullable<PUBGPresenceValidation['stats']>, sessionDurationMinutes: number): number {
    let bonusXP = 0;
    ...
    if (stats.kda > 2.0) bonusXP += 10;
    if (stats.wins > 100) bonusXP += 15;
    if (stats.rank !== 'Unranked' && stats.rank !== 'Bronze') bonusXP += 5;
    
    // Long session bonus
    if (sessionDurationMinutes >= 120) bonusXP += 20; // 2+ hours
    if (sessionDurationMinutes >= 240) bonusXP += 30; // 4+ hours
   ...
   */
  private async checkEnhancedNoShow(userId: string, guildId: string, sessionStartTime: Date): Promise<void> {
    try {
      const activeSession = this.presenceService.getActiveSession(guildId, userId);
      if (!activeSession) return; // User already checked out
   ...
   */
  private async checkEnhancedNoCheckout(userId: string, guildId: string, sessionStartTime: Date): Promise<void> {
    try {
      const activeSession = this.presenceService.getActiveSession(guildId, userId);
      if (!activeSession) return; // User already checked out
   ...
   */
  private async checkEnhancedEarlyLeave(userId: string, guildId: string, sessionStartTime: Date): Promise<void> {
    try {
      const activeSession = this.presenceService.getActiveSession(guildId, userId);
      if (activeSession) return; // User still in session
  ...
  private startAutoPunishmentMonitoring(): void {
    if (!this.autoPunishmentConfig.enabled) return;
    
    setInterval(async () => {
      await this.monitorActiveSessions();
    }, this.autoPunishmentConfig.checkInterval * 60 * 1000);
    
    this.logger.info(`Auto punishment monitoring started (interval: ${this.autoPunishmentConfig.checkInterval} minutes)`);
  }
      ...
      for (const [guildId, guild] of guilds) {
        ...
        for (const session of activeSessions) {
          ...
          if (sessionDuration > (8 * 60 * 60 * 1000)) { // 8 hours
            this.logger.warn(`Suspicious long session detected: User ${session.userId} in guild ${guildId} (${Math.round(sessionDuration / (60 * 60 * 1000))} hours)`);
   ...
   */
  private async sendSuspiciousActivityNotification(guildId: string, userId: string, duration: number): Promise<void> {
    try {
      const guild = this.client.guilds.cache.get(guildId);
      if (!guild) return;
  ...
  public getEnhancementStats(): {
    ...
    const validPubgUsers = Array.from(this.pubgValidations.values())
